...
let convertRGBAtoHSLA = (rgba: RGBA.t): HSLA.t => {
  let (red, green, blue, alpha) = (rgba->RGBA.red |> Int8.asInt, rgba->RGBA.green |> Int8.asInt, rgba->RGBA.blue |> Int8.asInt, rgba->RGBA.alpha |> Percent.asFloat);
...
let convertHSLAtoRGBA = (hsla: HSLA.t): RGBA.t => {
  let (h, s, l, a) = (hsla->HSLA.hue |> Degree.asFloat, hsla->HSLA.saturation |> Percent.asFloat, hsla->HSLA.lightness |> Percent.asFloat, hsla->HSLA.alpha |> Percent.asFloat);
  let clip_hue = (x) =>
  if (360.0 == x) {
    x;
  } else {
...
let convertHSLtoHSLA = (hsl: HSL.t): HSLA.t => {
  HSLA.make(~hue=hsl->HSL.hue, ~saturation=hsl->HSL.saturation, ~lightness=hsl->HSL.lightness, ~alpha=Percent.make(1.0));
};

let getLuminance = (color: color): float => {
  let rgb = switch(color) {
    | RGB(rgb) => rgb
